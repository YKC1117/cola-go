...
export function normalizeChargingStations(items, city) {
  return uniqueById(items.map((x, index) => {
    ...
    return { id, sourceId: sourceId || null, scope: `city:${city}`,
      name: localizedName(x.StationName) || localizedName(x.Name) || sourceId || "充電站",
      operatorId: x.OperatorID ?? x.OperatorId ?? null, address: x.Address ?? null, lat: p.lat, lon: p.lon,
      connectorSummary: connectorSummary(x.Connectors || x.ConnectorTypes),
      hoursText: x.ServiceTime ?? x.OperationTime ?? null, joinable: Boolean(sourceId) };
  }));
}
...
export function normalizeChargingPoints(items, city) {
  return items.map((x, index) => {
    ...
    return { id: sourceId ? `${city}:${sourceId}` : `${city}:point-unjoined-${index}`,
      sourceId: sourceId || null, stationId, operatorId: x.OperatorID ?? null,
      floor: x.Floor ?? x.FloorDescription ?? null, feeText: x.ChargingFee ?? x.FeeDescription ?? null,
      connectorSummary: connectorSummary(x.Connectors), joinable: Boolean(sourceId && stationId) };
  });
}
...
export function normalizeChargingConnectors(items, city) {
  return items.map((x, index) => {
    ...
    return { id: sourceId ? `${city}:${sourceId}` : `${city}:connector-unjoined-${index}`,
      sourceId: sourceId || null, stationId, chargingPointId,
      type: x.ConnectorType ?? x.Type ?? null, currentType: x.CurrentType ?? null,
      maxPowerKw: numberOrNull(x.MaxPower ?? x.MaxPowerKW ?? x.Power),
...
export function normalizeChargingAvailability(items, city) {
  return items.map((x, index) => {
    ...
    const rawState = x.Status ?? x.ConnectorStatus ?? x.AvailabilityStatus;
    return { id: sourceId ? `${city}:${sourceId}` : `${city}:availability-unjoined-${index}`,
      sourceId: sourceId || null, stationId, chargingPointId,
      connectorType: x.ConnectorType ?? x.Type ?? null, state: rawState ?? "unknown",
      sourceUpdatedAt: x.DataCollectTime ?? x.UpdateTime ?? null,
      identityQuality: joinable ? "connector-station-point" : "unreliable", joinable };
  });
}

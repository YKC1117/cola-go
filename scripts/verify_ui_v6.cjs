...
(async()=>{
  ...
  check('Direct navigation options include Google, Apple and Waze',await page.locator('#tripResult [data-map]').evaluateAll(nodes=>nodes.map(n=>n.dataset.map).sort().join(',')==='apple,google,waze'));
  await page.locator('#tripResult [data-map=waze]').click();
  check('Waze navigation uses entered destination directly',await page.evaluate(()=>{const u=new URL(window.__opened.at(-1));return u.hostname==='www.waze.com'&&u.searchParams.get('q')==='臺中車站'&&u.searchParams.get('navigate')==='yes';}));
